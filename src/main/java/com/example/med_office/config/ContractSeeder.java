...
public class ContractSeeder implements CommandLineRunner {
    ...
    public void run(String... args) throws Exception {
        System.out.println("=== Running ContractSeeder ===");

        // 1. Sync Contract catalog definitions to permissions table
        syncPermission(PermissionCatalog.EMPLOYEES_CONTRACT_EXPIRING_VIEW, "human-resources", "Nhân sự", "Xem danh sách nhân sự gần hết hạn hợp đồng");
        syncPermission(PermissionCatalog.EMPLOYEES_CONTRACT_MANAGE, "human-resources", "Nhân sự", "Quản lý hợp đồng lao động");

        // Get permissions from DB
        Optional<Permission> viewPermOpt = permissionRepository.findByCode(PermissionCatalog.EMPLOYEES_CONTRACT_EXPIRING_VIEW);
        Optional<Permission> managePermOpt = permissionRepository.findByCode(PermissionCatalog.EMPLOYEES_CONTRACT_MANAGE);

        if (viewPermOpt.isEmpty() || managePermOpt.isEmpty()) {
            System.err.println("Error: Contract permissions could not be synced.");
            return;
        }
        ...
        // 2. Assign EMPLOYEES_CONTRACT_EXPIRING_VIEW to all standard roles
        ...
        // 3. Assign EMPLOYEES_CONTRACT_MANAGE to ADMIN and NHAN_SU roles
        ...
        System.out.println("=== ContractSeeder Completed successfully ===");
    }
    ...
    private void syncPermission(String code, String moduleCode, String moduleName, String name) {
        if (permissionRepository.findByCode(code).isEmpty()) {
            ...
            System.out.println("Synced contract permission to DB: " + code);
        }
    }
